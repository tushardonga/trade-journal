    ...
    <div className="bg-white rounded-xl border shadow-sm overflow-hidden mb-8">
      ...
      <div className="grid grid-cols-7 border-b bg-slate-100">
        {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((d) => (
          <div
            key={d}
            className="py-2 text-center text-xs font-bold text-slate-500 uppercase"
          >
            {d}
          </div>
        ))}
            ...
            >
              ...
              {stats && (
                <div className="mt-1 flex flex-col items-center">
                  <span
                    className={`text-sm font-bold ${stats.pnl >= 0 ? "text-green-600" : "text-red-600"}`}
                  >
                    {Math.abs(stats.pnl) >= 1000
                      ? `${stats.pnl >= 0 ? "+" : "-"}${(Math.abs(stats.pnl) / 1000).toFixed(1)}k`
